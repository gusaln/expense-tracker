 ...
 */
export const getIncomeReport = async (after: Date, before: Date, accountId?: AccountDbId): Promise<BalanceByCurrency> => {
  const query = db
    .table('transactions')
 ...
 * @todo Handle multiple currencies
 */
export const getExpenseReport = async (after: Date, before: Date, accountId?: AccountDbId): Promise<BalanceByCurrency> => {
  const query = db
    .table('transactions')
    .innerJoin('accounts', 'accounts.id', '=', 'transactions.account_id')
