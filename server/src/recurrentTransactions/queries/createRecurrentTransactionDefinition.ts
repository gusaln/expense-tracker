...
import {
  ...
  RecurrentTransactionDefinitionNew
} from "../types";
 ...
 */
export default async function createRecurrentTransactionDefinition(data: RecurrentTransactionDefinitionNew,
  connection?: Knex) {
  const newRecur = await (connection || db)
  ...
  return transformDbRecordToRecurrentTransactionDefinition(newRecur[0]);
}
