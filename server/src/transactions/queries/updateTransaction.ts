...
import {
  IncomeOrExpenseTransactionType,
  Transaction,
  TransactionDbId,
  TransactionDbRecord,
  TransactionOfType,
...
/**
 * Persists changes made to a transaction
 *
 * @throws {ResourceNotFoundError} if the transaction does not exist.
 */
export const updateTransaction = async <T extends TransactionType>(
  id: TransactionDbId,
  data: Transaction<T> | TransactionUpdate<T>,
  connection ?: Knex
): Promise<TransactionOfType<T>> => {
  const oldData = "id" in data ? {...data} : await findTransactionById(id, connection);
  ...
  const updatedRecords =
    data.type == "transfer"
      ? await updateTransferTransaction(
          id,
          data as TransactionUpdate<TransferTransactionType>,
          amountDiff,
          connection
        )
      : await updateIncomeOrExpenseTransaction(
          id,
          data as TransactionUpdate<IncomeOrExpenseTransactionType>,
          amountDiff,
          connection
        );
...
const updateIncomeOrExpenseTransaction = async (
  ...
  return await (connection || db).transaction(async (trx) => {
...
const updateTransferTransaction = async (
  id: TransactionDbId,
  data: TransactionUpdate<TransferTransactionType>,
  amountDiff: number,
  connection ?: Knex
) => {
