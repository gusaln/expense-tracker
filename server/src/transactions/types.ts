...
import { ExpenseTransactionSchema, IncomeTransactionSchema, TransactionSchema, TransferTransactionSchema } from './schemas';

export type IncomeOrExpenseTransactionType = typeof TRANSACTION_TYPE_INCOME | typeof TRANSACTION_TYPE_EXPENSE;
...
export interface IncomeTransaction extends Omit<
  Static<typeof IncomeTransactionSchema>,
...
export interface ExpenseTransaction extends Omit<
  Static<typeof ExpenseTransactionSchema>,
  'date' | 'created_at' | 'updated_at'
> {
  date: Date,
  created_at: Date,
...
export interface TransferTransaction extends Omit<
  Static<typeof TransferTransactionSchema>,
  'date' | 'created_at' | 'updated_at'
> {
  date: Date,
  created_at: Date,
...
type ExtractTransactionType<T extends WithTransactionType> = T extends { type: TransactionType } ? T['type'] : T
...
/** Any transaction */
export interface Transaction<TType  extends TransactionType = TransactionType> extends Pick<
  Static<typeof TransactionSchema>,
  'id' | 'account_id' | 'description' | 'amount'
> {
  type: TType,
...
export type TransactionOfType<TType extends WithTransactionType> =
  ExtractTransactionType<TType> extends typeof TRANSACTION_TYPE_INCOME ? IncomeTransaction : (
    ExtractTransactionType<TType> extends typeof TRANSACTION_TYPE_EXPENSE ? ExpenseTransaction : (
      ExtractTransactionType<TType> extends typeof TRANSACTION_TYPE_TRANSFER ? TransferTransaction :
        Transaction
    )
  )
